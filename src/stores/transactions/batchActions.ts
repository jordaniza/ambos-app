...
export async function getCashNow(
  ...
  const [wethAddr, poolAddr, usdcAddr] = await Promise.all([
    promiseWeth,
    promisePool,
    promisUsdc,
  ]);
  ...
  const data0 = await weth.populateTransaction.approve(
    poolAddr,
    ethers.constants.MaxUint256,
  );
  const data1 = await pool.populateTransaction.supply(
    wethAddr,
    amountInWeth,
    borrower,
    0,
